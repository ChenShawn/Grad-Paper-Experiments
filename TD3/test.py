...
def multi_test_mass(arglist):
    """multi_test_mass
        Testing performances under various relative mass values
    ...
    log_filename = './logs/{}_{}_{}_multi_mass.log'.format(arglist.policy, arglist.env, arglist.test_seed)
...
def multi_test_noise(arglist):
    """multi_test_noise
        Testing performance using different scale of gaussian noise
        return: a list of dicts [{env, random_seed, noise_scale, mean, half_std}, {}, ...]
    """
    import gc
    noise_scale_list = np.arange(0.0, 1.50001, 0.05).tolist()
    log_filename = './logs/{}_{}_{}_multi_noise.log'.format(arglist.policy, arglist.env, arglist.test_seed)

    result_list = []
    log_fd = open(log_filename, 'w+')
    for noise_scale in noise_scale_list:
        arglist.noise_scale = noise_scale
        reward_list = test(arglist)

        reward_array = np.array(reward_list, dtype=np.float32)
        reward_mean = reward_array.mean()
        reward_half_std = reward_array.std() / 2.0
        loginfo = 'env={} random_seed={} noise_scale={} result={}±{}'
        print(loginfo.format(args.env, args.test_seed, noise_scale, reward_mean, reward_half_std))

        info_dict = {'env': arglist.env, 'random_seed': arglist.test_seed, 'noise_scale': noise_scale, 
                     'reward_mean': reward_mean, 'reward_half_std': reward_half_std}
        result_list.append(info_dict)
        log_fd.write(loginfo.format(args.env, args.test_seed, mass, reward_mean, reward_half_std) + '\n')
        log_fd.flush()
