...
do
    echo "pass"
    #python test.py -p adv -e ${env_name} -n 100 --train-seed ${seed} --test-seed ${seed} --relative-mass ${mass} >> ${mass_log}
...
do
    python test.py -p adv -e ${env_name} -n 25 --train-seed ${seed} --test-seed ${seed} --noise-scale ${scale} >> ${noise_log}
