...
do
    ...
    # relative mass test
    for mass in `seq 0.5 0.05 1.5`
    do
        python test.py -p adv -e LunarLanderContinuous-v2 -n 100 --train-seed ${seed} --test-seed ${seed} --relative-mass ${mass} >> ${mass_log}
        python test.py -p adv -e HalfCheetah-v2 -n 100 --train-seed ${seed} --test-seed ${seed} --relative-mass ${mass} >> ${mass_log}
    ...
    do
        python test.py -p adv -e LunarLanderContinuous-v2 -n 100 --train-seed ${seed} --test-seed ${seed} --noise-scale ${scale} >> ${noise_log}
        python test.py -p adv -e HalfCheetah-v2 -n 100 --train-seed ${seed} --test-seed ${seed} --noise-scale ${scale} >> ${noise_log}
